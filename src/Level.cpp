...
InvasionManager::InvasionManager(const json &data)
        : creepsRemaining_(0), currentWave_(-1) {
...
LevelInstance::LevelInstance(std::shared_ptr<Level> level)
        : level_(level), towerMap_(new std::shared_ptr<Tower>[level->getWidth() * level->getHeight()]),
          invasionManager_(level_->cloneInvasionManager()), gridNavigation_(*this, level->getGoal()),
          gridTowerPlacement_(*this), wavesRunning_(false), money_(level->getStartingMoney()),
...
void LevelInstance::update(sf::Time dt) {
    ...
    std::vector<std::list<std::shared_ptr<Decoration>>::iterator> toRemove;
...
void LevelInstance::addDecoration(std::shared_ptr<Decoration> decoration) {
